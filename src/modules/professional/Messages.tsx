...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('professional-conversations-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const loadConversations = async () => {
    ...
    const formatted = await Promise.all(
      convData.map(async (conv: any) => {
        ...
        const isUnread = lastMsg && !lastMsg.read && lastMsg.sender_id !== user.id;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white pb-20">
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-8 shadow-lg">
        ...
        <p className="text-emerald-100 mt-2 text-sm">Suas conversas com clientes</p>
      </div>
